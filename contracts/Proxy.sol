...
contract Proxy {
    ...
    constructor(address _addr) {
        StorageSlot.getStorage(ADMIN_SLOT).addr= msg.sender;
        StorageSlot.getStorage(IMPLEMENTATION_SLOT).addr= _addr;
    }

    //0x21bc166c
    function _setAdmin(address _admin) ifAdmin external {
        require(_admin != address(0), "admin = zero address");
        StorageSlot.getStorage(ADMIN_SLOT).addr = _admin;
    }

    //0x44f3454b
    function _setImplementation(address _addr) external ifAdmin {
        StorageSlot.getStorage(IMPLEMENTATION_SLOT).addr = _addr;
    }
